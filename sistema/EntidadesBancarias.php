...
<script type="text/javascript" language="javascript">
	...
	function actualizar(pagina,numero_registros,filtro,orden,filtro2,clave){
		$("#muestra").load("../recursos/funciones/ajaxEntidadesBancarias.php", {action:6, pagina: pagina, muestra: numero_registros , filtro_orden: filtro, orden: orden, filtro_busqueda: filtro2, clave: clave},function(){
			ajustar();	
		});
	}
	...
	function detalle(id){
		if ($("#detalle"+id).is(':hidden')){
			...
			$("#detalle"+id).load("../recursos/funciones/ajaxEntidadesBancarias.php", {action: 2, identificador: id},function(){
				$( "#estatus2" ).buttonset();
			 	$( "input[type=button]" )
			      .button()
	...
	function editar(id){						
		if ($("#detalle"+id).is(':hidden')){
			$(".tabla-detalle").slideUp("slow");
			$('.tabla-detalle-contenedor').empty();
			$("#detalle"+id).load("../recursos/funciones/ajaxEntidadesBancarias.php", {action: 4, identificador: id},function(){
				$( "#estatus3" ).buttonset();
			 	$( "input[type=button]" )
			      .button()
			      .click(function( event ) {
	...
	function agregar(){		
		...
		if ($("#agregar").is(':hidden')){
			$('.tabla-detalle-contenedor').empty();	
			$("#agregar").load("../recursos/funciones/ajaxEntidadesBancarias.php", {action: 3},function(){
				$( "#estatus1" ).buttonset();	
				$( "input[type=button]" )
				  .button()
				  .click(function( event ) {
  ...
  function eliminarbanco(codigo){
		...
		if (answer)
		location.href="../recursos/funciones/ajaxEntidadesBancarias.php?action=7&id="+document.getElementById("elimina").value;	    
  }
