...
<title>Rutas de Recolección de Leche</title>
</head>
...
<body onload="ajustar()">
    ...
    <div class="barra2">
        ...
        <div class="barra2_pantalla">Rutas de Recolección de Leche</div>
    </div>
	...
	<div class="cuerpo" id="cuerpo">
    	<div class="tabla">
	    	...
	    	<div class="tabla-titulo">Rutas de Recolección de Leche
	        ...
	        <div class="tabla-cabecera" style="border-bottom:0px;">                
		        ...
		        <input type="text" id="clave" name="clave" class="campo_filtro" placeholder="Indique su referencia" />
      			<select name="select_filtros" id="select_filtros" class="select_filtro" placeholder="Seleccione el campo para filtrar">
		            ...
		            <option value="idruta">Codigo</option>
		            <option value="nombreruta">Nombre de la Ruta</option>
		            <option value="encargado">Encargado</option>
		            <option value="rif">Documento</option>
		            <option value="pagounidad">Bs Litro</option>
		        </select>         
        ...
        <div class="tabla-cabecera">        
            	...
            	<div class="tabla-cabecera-elemento-flechas_arriba" title="Ordenar Ascendentemente" onclick=actualizar_filtros("idruta","asc")></div>
            	<div class="tabla-cabecera-elemento-flechas_abajo" title="Ordenar Descendentemente" onclick=actualizar_filtros("idruta","desc")></div>
            	</div>
          	</div>          
       	  	<div class="tabla-cabecera-elemento" style="width:26%;">Nombre de la Ruta
            	<div class="tabla-cabecera-elemento-flechas"><img src="../recursos/imagenes/0.png" style="position:absolute;" width="16" height="18" />
            	<div class="tabla-cabecera-elemento-flechas_arriba" title="Ordenar Ascendentemente" onclick=actualizar_filtros("nombreruta","asc")></div>
            	<div class="tabla-cabecera-elemento-flechas_abajo" title="Ordenar Descendentemente" onclick=actualizar_filtros("nombreruta","desc")></div>
            	</div>
          	</div>  
       	  	<div class="tabla-cabecera-elemento" style="width:26%;">Encargado
            	<div class="tabla-cabecera-elemento-flechas"><img src="../recursos/imagenes/0.png" style="position:absolute;" width="16" height="18" />
            	<div class="tabla-cabecera-elemento-flechas_arriba" title="Ordenar Ascendentemente" onclick=actualizar_filtros("encargado","asc")></div>
            	<div class="tabla-cabecera-elemento-flechas_abajo" title="Ordenar Descendentemente" onclick=actualizar_filtros("encargado","desc")></div>
            	</div>
          	</div>      
       	  	<div class="tabla-cabecera-elemento" style="width:13%;">Documento
            	<div class="tabla-cabecera-elemento-flechas"><img src="../recursos/imagenes/0.png" style="position:absolute;" width="16" height="18" />
            	<div class="tabla-cabecera-elemento-flechas_arriba" title="Ordenar Ascendentemente" onclick=actualizar_filtros("rif","asc")></div>
            	<div class="tabla-cabecera-elemento-flechas_abajo" title="Ordenar Descendentemente" onclick=actualizar_filtros("rif","desc")></div>
            	</div>
          	</div> 
       	  	<div class="tabla-cabecera-elemento" style="width:22%; border-right:0px;">Bs Litro
            	<div class="tabla-cabecera-elemento-flechas"><img src="../recursos/imagenes/0.png" style="position:absolute;" width="16" height="18" />
            	<div class="tabla-cabecera-elemento-flechas_arriba" title="Ordenar Ascendentemente" onclick=actualizar_filtros("pagounidad","asc")></div>
            	<div class="tabla-cabecera-elemento-flechas_abajo" title="Ordenar Descendentemente" onclick=actualizar_filtros("pagounidad","desc")></div>
            	</div>
          	</div>                                                             
        </div>
        ...
        <?php
		    $sql_registros="select * from ruta order by idruta";
...
<script type="text/javascript" language="javascript">
	...
	function actualizar(pagina,numero_registros,filtro,orden,filtro2,clave){
		$("#muestra").load("../recursos/funciones/ajaxRutasDeRecoleccionDeLeche.php", {action:6, pagina: pagina, muestra: numero_registros , filtro_orden: filtro, orden: orden, filtro_busqueda: filtro2, clave: clave},function(){
			ajustar();	
		});
	}
	...
	function detalle(id){
		if ($("#detalle"+id).is(':hidden')){
			...
			$("#detalle"+id).load("../recursos/funciones/ajaxRutasDeRecoleccionDeLeche.php", {action: 2, identificador: id},function(){
				$( "#estatus2" ).buttonset();
				$("#tipopersona2").chosen({no_results_text: "No se han encontrado resultados para: "});
			 	$( "input[type=button]" )
			      .button()
	...
	function editar(id){						
		if ($("#detalle"+id).is(':hidden')){
			$(".tabla-detalle").slideUp("slow");
			$('.tabla-detalle-contenedor').empty();
			$("#detalle"+id).load("../recursos/funciones/ajaxRutasDeRecoleccionDeLeche.php", {action: 4, identificador: id},function(){
				$( "#estatus3" ).buttonset();
				$("#tipopersona3").chosen({no_results_text: "No se han encontrado resultados para: "});
			 	$( "input[type=button]" )
			      .button()
			      .click(function( event ) {
		          event.preventDefault();
	...
	function agregar(){		
		...
		if ($("#agregar").is(':hidden')){
			$('.tabla-detalle-contenedor').empty();	
			$("#agregar").load("../recursos/funciones/ajaxRutasDeRecoleccionDeLeche.php", {action: 3},function(){
				$( "#estatus1" ).buttonset();
				...
				$("#tipopersona1").chosen({no_results_text: "No se han encontrado resultados para: "});	
				document.getElementById("tipopersona1_chzn").style.width="174px";				        
  ...
  function agregarregistro(){
	    ...
	    if(document.getElementById("nombre1").value=="" && band==0){
			 alert("El nombre de la ruta no puede quedar vacio.");			 
			 band=1;    	 	
		}
	    if(document.getElementById("encargado1").value=="" && band==0){	
		     alert("El nombre del encargado no puede quedar vacio.");			 
			 band=1;    	 	
		}
	    ...
	    if(document.getElementById("tipopersona1").value=="" && band==0){	
			 alert("Debe indicar si la ruta pertenece a una persona natural o juridica.");		 
			 band=1;    	 	
		}				
	    if(document.getElementById("placa1").value=="" && band==0){	
			 alert("La placa de la ruta no puede quedar vacia.");		 
			 band=1;    	 	
		}	
	    if(document.getElementById("capacidad1").value=="" && band==0){		
			 alert("La capacidad máxima de la ruta no puede quedar vacia.");		 
			 band=1;    	 	
		}	
		
		if(!/^([0-9])*[.]?[0-9]*$/.test(document.getElementById("capacidad1").value)){
			 alert("La capacidad máxima introducida no cumple con el formato requerido, debe ser un numero.");			 
			 band=1; 		   	        
		}			
  ...
  function editarregistro(){
	    ...
	    if(document.getElementById("encargado3").value=="" && band==0){		 
			 alert("El nombre del encargado no puede quedar vacio.");
			 band=1;    	 	
		}
	    ...
	    if(document.getElementById("tipopersona3").value=="" && band==0){	
			 alert("Debe indicar si la ruta pertenece a una persona natural o juridica.");		 
			 band=1;    	 	
		}				
	    if(document.getElementById("placa3").value=="" && band==0){	
			 alert("La placa de la ruta no puede quedar vacia.");			 
			 band=1;    	 	
		}	
	    if(document.getElementById("capacidad3").value=="" && band==0){	
			 alert("La capacidad máxima de la ruta no puede quedar vacia.");			 
			 band=1;    	 	
		}	
		
		if(!/^([0-9])*[.]?[0-9]*$/.test(document.getElementById("capacidad3").value)){
			 alert("La capacidad máxima introducida no cumple con el formato requerido, debe ser un numero.");			 
			 band=1; 		   	        
		}			
  ...
  function eliminarregistro(codigo){
		var answer = confirm ("¿Esta seguro que desea eliminar este Ruta de Recolección de Leche?")
		if (answer)
		location.href="../recursos/funciones/ajaxRutasDeRecoleccionDeLeche.php?action=7&id="+document.getElementById("elimina").value;		    
  }
